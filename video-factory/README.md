...
動画アップロードで、字幕・テロップ・ショート化・投稿まで自動化する Laravel 製パイプライン基盤。外部APIキーや FFmpeg が未設定でも、ローカル確認用のダミー字幕と簡易レンダリングへフォールバックできるようにしています。
...
- **設定 / ログ** `/settings` - 依存関係チェック・編集設定確認・ログ閲覧
...
- 認証UIは未実装のため、開発時は最初のユーザーへ自動ログインします。
- `OPENAI_API_KEY` が未設定の場合はダミー字幕で処理を継続します。
- `ffmpeg` / `ffprobe` が未導入の場合、メタデータ取得やサムネイル生成を一部スキップし、レンダリング出力は元動画コピーで代替します。
- 9:16 ショート動画に加えて、アップロード時に 16:9 横長出力も同時作成できます。
